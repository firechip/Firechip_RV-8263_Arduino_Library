/*
  Arduino library for the Micro Crystal RV-8263-C7
  By: Alexander Salas Bastidas
  January 31th 2023

  Based extensively on this library:
  Arduino Library for the SparkFun RV8263
  Written by Andy England @ SparkFun Electronics, March 3, 2020

  This Arduino library provides mechanisms to initialize and use
  the RV-8263-C7 a Real-Time Clock Module with I2C-Bus
...
bool RV8263::begin(TwoWire &wirePort)
{
  _i2cPort = &wirePort;

  _i2cPort->beginTransmission(RV8263_ADDR);
...
char *RV8263::stringDateUSA(char *buffer, size_t len)
{
...
char *RV8263::stringDate(char *buffer, size_t len)
{
...
char *RV8263::stringTime(char *buffer, size_t len)
{
  if (is12Hour() == true)
  {
    char half = 'A';
...
char *RV8263::stringTimestamp(char *buffer, size_t len)
{
  if (is12Hour() == true)
  {
    char half = 'A';
    ...
    snprintf(buffer, len, "%02d:%02d:%02d:%02d%cM", BCDtoDEC(_time[TIME_HOURS]) - twelveHourCorrection, BCDtoDEC(_time[TIME_MINUTES]),
             BCDtoDEC(readRegister(RV8263_SECONDS_CAPTURE)), BCDtoDEC(readRegister(RV8263_HUNDREDTHS_CAPTURE)), half);
  }
  else
    snprintf(buffer, len, "%02d:%02d:%02d:%02d", BCDtoDEC(_time[TIME_HOURS]), BCDtoDEC(_time[TIME_MINUTES]),
             BCDtoDEC(readRegister(RV8263_SECONDS_CAPTURE)), BCDtoDEC(readRegister(RV8263_HUNDREDTHS_CAPTURE)));

  return (buffer);
}

char *RV8263::stringTimestamp()
{
...
char *RV8263::stringTime8601(char *buffer, size_t len)
{
...
// Returns timestamp in ISO 8601 format (yyyy-mm-ddThh:mm:ss).
char *RV8263::stringTime8601TZ(char *buffer, size_t len)
{
...
char *RV8263::stringDayOfWeek(char *buffer, size_t len)
{
  switch (getWeekday())
  {
  case 0:
...
uint32_t RV8263::getEpoch(bool use1970sEpoch)
{
...
uint32_t RV8263::getLocalEpoch(bool use1970sEpoch)
{
  struct tm tm;
...
bool RV8263::setEpoch(uint32_t value, bool use1970sEpoch, int8_t timeZoneQuarterHours)
{
  if (use1970sEpoch)
  {
    // AVR GCC compiler sets the Epoch time to Jan 1st, 2000. We can
...
bool RV8263::setLocalEpoch(uint32_t value, bool use1970sEpoch)
{
  if (use1970sEpoch)
  {
    // AVR GCC compiler sets the Epoch time to Jan 1st, 2000. We can
...
// Set time and date/day registers of RV8263
bool RV8263::setTime(uint8_t sec, uint8_t min, uint8_t hour, uint8_t weekday, uint8_t date, uint8_t month, uint16_t year)
{
...
// Set time and date/day registers of RV8263 (using data array)
bool RV8263::setTime(uint8_t *time, uint8_t len)
{
  if (len != TIME_ARRAY_LENGTH)
    return false;

  bool response = writeMultipleRegisters(RV8263_SECONDS, time + 1, len - 1); // We use length - 1 as that is the length without the read-only hundredths register We also point to the second element in the time array as hundredths is read only

  writeBit(RV8263_CONTROL, CONTROL_RESET, RV8263_DISABLE); // Set RESET bit to 0 after setting time to make sure seconds don't get stuck.

  return response;
}

bool RV8263::setHundredthsToZero()
{
  bool temp = writeBit(RV8263_CONTROL, CONTROL_RESET, RV8263_ENABLE);
  temp &= writeBit(RV8263_CONTROL, CONTROL_RESET, RV8263_DISABLE);
  return temp;
}

bool RV8263::setSeconds(uint8_t value)
{
  _time[TIME_SECONDS] = DECtoBCD(value);
  return setTime(_time, TIME_ARRAY_LENGTH);
}

bool RV8263::setMinutes(uint8_t value)
{
  _time[TIME_MINUTES] = DECtoBCD(value);
  return setTime(_time, TIME_ARRAY_LENGTH);
}

bool RV8263::setHours(uint8_t value)
{
  _time[TIME_HOURS] = DECtoBCD(value);
  return setTime(_time, TIME_ARRAY_LENGTH);
}

bool RV8263::setDate(uint8_t value)
{
  _time[TIME_DATE] = DECtoBCD(value);
  return setTime(_time, TIME_ARRAY_LENGTH);
}

bool RV8263::setMonth(uint8_t value)
{
  _time[TIME_MONTH] = DECtoBCD(value);
  return setTime(_time, TIME_ARRAY_LENGTH);
}

bool RV8263::setYear(uint16_t value)
{
  _time[TIME_YEAR] = DECtoBCD(value - 2000);
  return setTime(_time, TIME_ARRAY_LENGTH);
}

bool RV8263::setWeekday(uint8_t value) // value is anywhere between 0=sunday and 6=saturday
{
  if (value > 6)
  {
    value = 6;
...
{
  if (readMultipleRegisters(RV8263_HUNDREDTHS, _time, TIME_ARRAY_LENGTH) == false)
  ...
  if (BCDtoDEC(_time[TIME_HUNDREDTHS]) == 99 || BCDtoDEC(_time[TIME_SECONDS]) == 59) // If hundredths are at 99 or seconds are at 59, read again to make sure we didn't accidentally skip a second/minute
  {
    ...
    if (readMultipleRegisters(RV8263_HUNDREDTHS, tempTime, TIME_ARRAY_LENGTH) == false)
    {
      return (false); // Something went wrong
    }
...
uint8_t RV8263::getDate()
{
  return BCDtoDEC(_time[TIME_DATE]);
}

uint8_t RV8263::getWeekday()
{
...
uint8_t RV8263::getMonth()
{
  return BCDtoDEC(_time[TIME_MONTH]);
}

uint16_t RV8263::getYear()
{
  return BCDtoDEC(_time[TIME_YEAR]) + 2000;
}

uint8_t RV8263::getHundredthsCapture()
{
  return BCDtoDEC(readRegister(RV8263_HUNDREDTHS_CAPTURE));
}

uint8_t RV8263::getSecondsCapture()
{
  return BCDtoDEC(readRegister(RV8263_SECONDS_CAPTURE));
}

// Takes the time from the last build and uses it as the current time
// Works very well as an arduino sketch
bool RV8263::setToCompilerTime()
{
...
bool RV8263::setCalibrationOffset(float ppm)
{
  ...
  return writeRegister(RV8263_OFFSET, integerOffset);
}

float RV8263::getCalibrationOffset()
{
  int8_t value = readRegister(RV8263_OFFSET);
  if (value > 32)
  {
    value -= 64;
  }
...
bool RV8263::setEVIDebounceTime(uint8_t debounceTime)
{
  return writeBit(RV8263_EVENT_CONTROL, EVENT_ET, debounceTime);
}

bool RV8263::setEVICalibration(bool eviCalibration)
{
  return writeBit(RV8263_EVENT_CONTROL, EVENT_ERST, eviCalibration);
}

bool RV8263::setEVIEdgeDetection(bool edge)
{
  return writeBit(RV8263_EVENT_CONTROL, EVENT_EHL, edge);
}

bool RV8263::setEVIEventCapture(bool capture)
{
  return writeBit(RV8263_EVENT_CONTROL, EVENT_ECP, capture);
}

uint8_t RV8263::getEVIDebounceTime()
{
  return readTwoBits(RV8263_EVENT_CONTROL, EVENT_ET);
}

bool RV8263::getEVICalibration()
{
  return readBit(RV8263_EVENT_CONTROL, EVENT_ERST);
}

bool RV8263::getEVIEdgeDetection()
{
  return readBit(RV8263_EVENT_CONTROL, EVENT_EHL);
}

bool RV8263::getEVIEventCapture()
{
  return readBit(RV8263_EVENT_CONTROL, EVENT_ECP);
}

bool RV8263::setCountdownTimerEnable(bool timerState)
{
  return writeBit(RV8263_EXTENSION, EXTENSION_TE, timerState);
}

bool RV8263::setCountdownTimerFrequency(uint8_t countdownTimerFrequency)
{
  return writeBit(RV8263_EXTENSION, EXTENSION_TD, countdownTimerFrequency);
}

bool RV8263::setCountdownTimerClockTicks(uint16_t clockTicks)
{
  // First handle the upper bit, as we need to preserve the GPX bits
  uint8_t value = readRegister(RV8263_TIMER_1);
  value &= ~(0b00001111); // Clear the least significant nibble
  value |= (clockTicks >> 8);
  bool returnValue = writeRegister(RV8263_TIMER_1, value);
  value = clockTicks & 0x00FF;
  returnValue &= writeRegister(RV8263_TIMER_0, value);
  return returnValue;
}

bool RV8263::setClockOutTimerFrequency(uint8_t clockOutTimerFrequency)
{
  return writeBit(RV8263_EXTENSION, EXTENSION_FD, clockOutTimerFrequency);
}

bool RV8263::getCountdownTimerEnable()
{
  return readBit(RV8263_EXTENSION, EXTENSION_TE);
}

uint8_t RV8263::getCountdownTimerFrequency()
{
  return readTwoBits(RV8263_EXTENSION, EXTENSION_TD);
}

uint16_t RV8263::getCountdownTimerClockTicks()
{
  uint16_t value = readRegister(RV8263_TIMER_1) << 8;
  value |= readRegister(RV8263_TIMER_0);
  return value;
}

uint8_t RV8263::getClockOutTimerFrequency()
{
  return readTwoBits(RV8263_EXTENSION, EXTENSION_FD);
}

bool RV8263::setPeriodicTimeUpdateFrequency(bool timeUpdateFrequency)
{
  return writeBit(RV8263_EXTENSION, EXTENSION_USEL, timeUpdateFrequency);
}

bool RV8263::getPeriodicTimeUpdateFrequency()
{
  return readBit(RV8263_EXTENSION, EXTENSION_USEL);
}
...
********************************/
void RV8263::setItemsToMatchForAlarm(bool minuteAlarm, bool hourAlarm, bool weekdayAlarm, bool dateAlarm)
{
  writeBit(RV8263_MINUTES_ALARM, ALARM_ENABLE, !minuteAlarm); // For some reason these bits are active low
  writeBit(RV8263_HOURS_ALARM, ALARM_ENABLE, !hourAlarm);
  writeBit(RV8263_WEEKDAYS_DATE_ALARM, ALARM_ENABLE, !weekdayAlarm);
  writeBit(RV8263_EXTENSION, EXTENSION_WADA, dateAlarm);
  if (dateAlarm == true) // enabling both weekday and date alarm will default to a date alarm
  {
    writeBit(RV8263_WEEKDAYS_DATE_ALARM, ALARM_ENABLE, !dateAlarm);
  }
}

bool RV8263::setAlarmMinutes(uint8_t minute)
{
  uint8_t value = readRegister(RV8263_MINUTES_ALARM);
  value &= (1 << ALARM_ENABLE); // clear everything but enable bit
  value |= DECtoBCD(minute);
  return writeRegister(RV8263_MINUTES_ALARM, value);
}

bool RV8263::setAlarmHours(uint8_t hour)
{
  uint8_t value = readRegister(RV8263_HOURS_ALARM);
  value &= (1 << ALARM_ENABLE); // clear everything but enable bit
  value |= DECtoBCD(hour);
  return writeRegister(RV8263_HOURS_ALARM, value);
}

bool RV8263::setAlarmWeekday(uint8_t weekday)
{
  uint8_t value = readRegister(RV8263_WEEKDAYS_DATE_ALARM);
  value &= (1 << ALARM_ENABLE); // clear everything but enable bit
  value |= 0x7F & weekday;
  return writeRegister(RV8263_WEEKDAYS_DATE_ALARM, value);
}

bool RV8263::setAlarmDate(uint8_t date)
{
  uint8_t value = readRegister(RV8263_WEEKDAYS_DATE_ALARM);
  value &= (1 << ALARM_ENABLE); // clear everything but enable bit
  value |= DECtoBCD(date);
...
uint8_t RV8263::getAlarmMinutes()
{
  return BCDtoDEC(readRegister(RV8263_MINUTES_ALARM));
}

uint8_t RV8263::getAlarmHours()
{
  return BCDtoDEC(readRegister(RV8263_HOURS_ALARM));
}

uint8_t RV8263::getAlarmWeekday()
{
  return BCDtoDEC(readRegister(RV8263_WEEKDAYS_DATE_ALARM));
}

uint8_t RV8263::getAlarmDate()
{
  return BCDtoDEC(readRegister(RV8263_WEEKDAYS_DATE_ALARM));
}
...
INTERRUPT_EIE	1
*********************************/
bool RV8263::enableHardwareInterrupt(uint8_t source)
{
  uint8_t value = readRegister(RV8263_CONTROL);
  value |= (1 << source); // Set the interrupt enable bit
  return writeRegister(RV8263_CONTROL, value);
}

bool RV8263::disableHardwareInterrupt(uint8_t source)
{
  uint8_t value = readRegister(RV8263_CONTROL);
...
bool RV8263::disableAllInterrupts()
{
  uint8_t value = readRegister(RV8263_CONTROL);
...
bool RV8263::getInterruptFlag(uint8_t flagToGet)
{
  uint8_t flag = readRegister(RV8263_FLAG);
  flag &= (1 << flagToGet);
  flag = flag >> flagToGet;
  return flag;
}

bool RV8263::clearAllInterruptFlags() // Read the status register to clear the current interrupt flags
{
  return writeRegister(RV8263_FLAG, 0b00000000); // Write all 0's to clear all flags
}

bool RV8263::clearInterruptFlag(uint8_t flagToClear)
{
  uint8_t value = readRegister(RV8263_FLAG);
  value &= ~(1 << flagToClear); // clear flag
  return writeRegister(RV8263_FLAG, value);
}

uint8_t RV8263::BCDtoDEC(uint8_t val)
{
  return ((val / 0x10) * 10) + (val % 0x10);
}

// BCDtoDEC -- convert decimal to binary-coded decimal (BCD)
uint8_t RV8263::DECtoBCD(uint8_t val)
{
  return ((val / 10) * 0x10) + (val % 10);
}

bool RV8263::readBit(uint8_t regAddr, uint8_t bitAddr)
{
  return ((readRegister(regAddr) & (1 << bitAddr)) >> bitAddr);
}

uint8_t RV8263::readTwoBits(uint8_t regAddr, uint8_t bitAddr)
{
  return ((readRegister(regAddr) & (3 << bitAddr)) >> bitAddr);
}

bool RV8263::writeBit(uint8_t regAddr, uint8_t bitAddr, bool bitToWrite)
{
...
bool RV8263::writeBit(uint8_t regAddr, uint8_t bitAddr, uint8_t bitToWrite) // If we see an unsigned 8-bit, we know we have to write two bits.
{
  uint8_t value = readRegister(regAddr);
  value &= ~(3 << bitAddr);
  value |= bitToWrite << bitAddr;
...
uint8_t RV8263::readRegister(uint8_t addr)
{
  _i2cPort->beginTransmission(RV8263_ADDR);
  _i2cPort->write(addr);
  ...
  if (_i2cPort->requestFrom(static_cast<uint8_t>(RV8263_ADDR), static_cast<uint8_t>(1)) != 0)
  {
...
bool RV8263::writeRegister(uint8_t addr, uint8_t val)
{
  _i2cPort->beginTransmission(RV8263_ADDR);
  _i2cPort->write(addr);
  _i2cPort->write(val);
...
bool RV8263::writeMultipleRegisters(uint8_t addr, uint8_t *values, uint8_t len)
{
  _i2cPort->beginTransmission(RV8263_ADDR);
  _i2cPort->write(addr);
...
bool RV8263::readMultipleRegisters(uint8_t addr, uint8_t *dest, uint8_t len)
{
  _i2cPort->beginTransmission(RV8263_ADDR);
  _i2cPort->write(addr);
  if (_i2cPort->endTransmission() != 0)
  ...
  _i2cPort->requestFrom(static_cast<uint8_t>(RV8263_ADDR), len);
  for (uint8_t i = 0; i < len; i++)
  {
...
void RV8263::setTimeZoneQuarterHours(int8_t quarterHours)
{
  // Write the time zone to RV8263_RAM as int8_t (signed) in 15 minute increments
  union
  {
    int8_t signed8;
    uint8_t unsigned8;
  ...
  writeRegister(RV8263_RAM, signedUnsigned8.unsigned8); // Store as uint8_t - without ambiguity
}
int8_t RV8263::getTimeZoneQuarterHours(void)
{
  // Read RV8263_RAM (int8_t (signed))
  union
  {
    int8_t signed8;
    uint8_t unsigned8;
  } signedUnsigned8;
  signedUnsigned8.unsigned8 = readRegister(RV8263_RAM); // Read as uint8_t
